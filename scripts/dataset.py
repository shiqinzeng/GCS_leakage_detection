...
import math
import torch
from torchvision.transforms import ToTensor, RandomHorizontalFlip, RandomVerticalFlip, ColorJitter, RandomRotation
...
def form_dataset(filename, number_of_samples):
    ...
    all_images = np.zeros((number_of_samples, 341, 650, 1))
...
def form_dataloaders(data):
    ...
    # Apply transformations only for the training dataset
    transformed_train_dataset = TransformedDataset(train_dataset, train_transforms)

    # Concatenate original training dataset with transformed dataset
    ...
    trainloader = DataLoader(combined_train_dataset, batch_size=8, shuffle=True, num_workers=4, pin_memory=True)
    trainloader = DataLoader(train_dataset, batch_size=8, shuffle=True, num_workers=4, pin_memory=True)
